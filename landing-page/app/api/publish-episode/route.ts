...
export async function POST(req: NextRequest) {
  ...
  if (!response.ok) {
    return NextResponse.json({ error: "Buzzsprout error", details: data }, { status: response.status });
  }

  const episodeUrl = `https://www.buzzsprout.com/${podcastId}/${data.id}`;
  return NextResponse.json({ success: true, episode: data, episode_url: episodeUrl });
}
